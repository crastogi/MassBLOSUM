...
public:
  ...
  IntegerMatrix computeDistance(std::vector<std::string>);
...
IntegerMatrix MassBLOSUM::computeDistance(std::vector<std::string> inStrings) {
  int score = 0;
  ...
  for (int i=0; i<inStrings.size(); i++) {
    for (int j=i; j<inStrings.size(); j++) {
      // Compute BLOSUM score for one sequence on the other
      if (inStrings[i].size()!=inStrings[j].size()) {
        // String sizes don't match. Fill with NA
        output(i,j) = NA_INTEGER;
        output(j,i) = NA_INTEGER;
      } else {
        // String sizes match. Compute BLOSUM score
        score = 0;
        for (int k=0; k<inStrings[i].size(); k++) {
          score += scoringMatrix(matchChar(inStrings[i][k]), matchChar(inStrings[j][k]));
        }
        output(i,j) = score;
        output(j,i) = score;
      }
